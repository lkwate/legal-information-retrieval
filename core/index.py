...
def init_encoder(encoder: str, device: torch.device):
    ...
    if "dpr" in encoder:
        ...
        return LongAutoDocumentEncoder(
            encoder, device=device, l2_norm=True, pooling="mean"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--encoder", type=str, help="encoder name or path", required=True
    )
    parser.add_argument(
        "--dimension",
        type=int,
        help="dimension of passage embeddings",
        required=False,
        default=768,
    )
    parser.add_argument(
